...
2. **`pacus_growth` e `task_events` não têm nenhum endpoint de leitura** — são logs internos, o que é bom para minimização de dados exposta via API, mas significa que não há hoje uma forma do adulto auditar essas informações pelo próprio app (relevante para o B2, exportação de dados: se a exportação for "tudo que sabemos sobre a família", esses dois precisam entrar).
