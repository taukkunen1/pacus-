...
**Todos os 22 itens concluídos** (atualizado em 2026-09-08):

- rate limiting, testes de isolamento por família, correção de um bug real de checagem de posse em `TasksController.Delete`, renomeação `UserId → FamilyId`, log de auditoria, mapa de dados das 12 collections, exportação de dados (LGPD, portabilidade), exclusão de conta (hard delete + anonimização de `audit_logs` com TTL de 12 meses), Política de Privacidade/Termos de Uso publicados no site, RIPD, Plano de Resposta a Incidentes;
- hardening de infra confirmado e corrigido (A6): `ASPNETCORE_ENVIRONMENT=Production`, `MONGODB_URI` migrado para o usuário `pacus_app` (privilégio mínimo, usuário antigo `atlasAdmin` removido), `CORS_ALLOWED_ORIGINS` com os dois frontends reais — feito originalmente no Render, revalidado no Fly.io após a migração de 2026-09-09 (ver seção de migração abaixo);
- histórico do Git auditado por segredo vazado, sem achados reais (A7);
- os dois tokens de acesso pessoal do GitHub expostos numa conversa anterior foram revogados (A8);
- canal de contato de privacidade (D4) público na Política de Privacidade, já visível no site desde a publicação do B5.

**Única ressalva, decisão de produto e não pendência técnica:** o Network Access do MongoDB Atlas permanece com `0.0.0.0/0` liberado — nem o Render nem o Fly.io (hospedagem atual da API, ver seção de migração abaixo) oferecem IP de saída fixo grátis, e o risco foi aceito conscientemente pelo responsável técnico dado o estágio atual do produto.
...
**Diagnóstico da lentidão (histórico — válido para o Render, hospedagem da época):** não era bug de código — era comportamento normal de infraestrutura em plano gratuito. O frontend (`frontend/`) é hospedado como site estático no GitHub Pages (`.github/workflows/pages.yml`, deploy só em push pra `main`) — isso carrega instantâneo, sempre. Quem hibernava era a **API no Render** (`pacus.onrender.com`): medido diretamente naquela sessão, uma chamada a `/api/v1/health` depois de um tempo sem uso levou **5.6s**, contra **0.3s** nas chamadas seguintes (já "acordada"). Esse era o padrão conhecido do plano gratuito do Render — o serviço dormia depois de um período sem tráfego e o primeiro request seguinte pagava o custo de acordar o container (em planos gratuitos isso costumava passar de 30s quando o sono era mais longo que o medido ali). Não havia nada de anormal no `Dockerfile` ou no `Program.cs` (nenhuma chamada bloqueante extra no boot) causando lentidão adicional além do "acordar" em si. **Esse diagnóstico ficou obsoleto em 2026-09-09 com a migração para o Fly.io — ver seção abaixo.**

**Correção aplicada (frontend, já que a causa raiz era infraestrutura fora do meu acesso):** antes, `fetch` em `api-client.js` não tinha nenhum timeout nem feedback visual — durante um cold start, a tela só mostrava "Carregando..." parado (tela de login) ou o botão "Entrar" sem nenhuma mudança visível, o que parece travado/quebrado em vez de "o servidor está acordando". Adicionado:
...
- Esse aviso continua útil mesmo após a migração pro Fly.io: o plano gratuito de lá também hiberna a máquina sem tráfego (ver seção de migração abaixo), então o cold start não desapareceu, só mudou de provedor.
...
## Migração da API do Render para o Fly.io (2026-09-09)

A pedido do dono do produto, migrado o backend de produção do Render para o Fly.io, via GitHub-integrated deploy (dashboard do Fly.io, sem CLI disponível nesta sessão). Resumo:

- **Motivo:** no plano gratuito do Render a API "dorme" após inatividade (cold start de alguns segundos a mais de 30s, ver seção de lentidão acima). O Fly.io foi escolhido como alternativa; foi adicionado cartão de crédito na conta do Fly.io para evitar o limite de "5 minutos de execução por trial" que as máquinas sem cartão têm lá (achado direto nos logs do Fly durante a migração).
- **App:** `pacus-pacus-api` (config em `backend/fly.toml`), região `iad` (Ashburn, EUA) — escolhida por ser uma das regiões mais baratas do Fly.io e por ser a região primária de infraestrutura da empresa (start mais rápido); São Paulo (`gru`) tem cerca de 60% de markup de preço lá, então foi descartada a favor de custo/velocidade sobre proximidade geográfica, a pedido do dono do produto.
- **Configuração:** "Current Working Directory" = `backend` (o projeto .NET vive num subdiretório, não na raiz do repo), IP público (IPv4 compartilhado, gratuito) atribuído, segredos (`MONGODB_URI`, `JWT_SECRET`, `CORS_ALLOWED_ORIGINS`) configurados diretamente no dashboard do Fly.io pelo dono do produto, deploy automático a cada push em `main` ("Auto-Deploy on push").
- **Frontend:** `window.PACUS_API_BASE_URL` em `frontend/index.html` atualizado de `https://pacus.onrender.com/api/v1` para `https://pacus-pacus-api.fly.dev/api/v1` (PR #20, mesclado em `main`), refletido tanto no GitHub Pages (`taukkunen1.github.io`) quanto no espelho estático hospedado no Render (`pacus-1.onrender.com`, que faz o próprio auto-deploy a partir do mesmo repositório).
- **Validação de ponta a ponta feita na nova API**, direto do frontend real em produção: criação de família de teste (`POST /bootstrap`) → login do adulto (`POST /auth/adult/login`) → leitura autenticada (`GET /family/code`) → exclusão da conta de teste (`DELETE /account`). Todos os passos com o status HTTP esperado (201, 200, 200, 204) e sem lixo deixado no MongoDB Atlas.
- **Backend antigo no Render (`pacus.onrender.com`):** suspenso em 2026-09-10 a pedido do dono do produto — decisão de usar somente o Fly.io daqui pra frente pra API. Serviço suspenso não é cobrado e continua existindo no painel do Render (pode ser reativado em "Resume service" se necessário), mas não recebe mais tráfego nem faz auto-deploy. **O espelho estático do frontend no Render (`pacus-1.onrender.com`) permanece ativo** — o dono do produto pediu explicitamente para não suspender esse site, só a API; ele continua fazendo auto-deploy a partir do mesmo repositório, em paralelo ao GitHub Pages.
